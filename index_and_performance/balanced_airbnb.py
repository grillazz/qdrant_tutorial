...
config = {"name": "balanced_airbnb", "m": 8, "ef_construct": 100}  # m=0 = ingest-only
...
def upload_batch_without_indexes(start_idx, end_idx):
    ...
    for i in range(start_idx, min(end_idx, total_points)):
        ...
        # Get the embedding
        embedding = example['text_embeddings']
        ...
        payload = {
            ...
            'title': example['name'],
            '_id': example['_id'],
            'length': len(example['space']),
            'has_numbers': any(char.isdigit() for char in example['space'])
        }
