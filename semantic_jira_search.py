...
MAX_TOKENS = 60
collection = f'jira_task_search_{model}_{MAX_TOKENS}'
...
# 256 bad ide as 1st and 2nd are not good match
...
# Global instance to avoid reloading weights
semantic_embed_model = HuggingFaceEmbedding(model_name=f"sentence-transformers/{model}")
...
jira_data = documents = [
    ...
    },
    {
        ...
        "description": "Use a soil moisture probe or sensor to measure moisture levels in the root zone around peach trees,"
                       " focusing on representative sites within the orchard. Compare readings with historical data and current weather conditions"
                       " to determine whether irrigation is needed or should be reduced. If soil is drying too quickly"
                       " adjust the irrigation schedule or flow rate to maintain consistent moisture without waterlogging."
                       " Record any areas that consistently dry out or remain too wet, as these may indicate uneven distribution or drainage issues."
                       " Maintaining proper soil moisture is crucial for fruit set, fruit size, and overall tree health,"
                       " especially during key growth stages like flowering and fruit expansion.",
    ...
    },
    {
        ...
        "description": "Water pepper seedlings at the base using a gentle flow or drip method,"
                       " ensuring that the root zone is moist without saturating the soil."
                       " Avoid overhead watering that wets the foliage excessively, as this can encourage fungal diseases."
                       " Check the soil moisture daily or every other day, depending on weather and soil type, and adjust the amount or frequency as needed."
                       " As the plants grow, gradually increase the volume of water to match their expanding root system. Adequate moisture during this early stage supports strong root establishment, reduces transplant shock, and helps peppers develop sturdy stems and healthy leaves before flowering begins.",
    ...
    },
    {
        ...
        "description": "Remove any compacted, matted, or decomposed mulch from around blueberry bushes,"
                       " taking care not to disturb the shallow root system during the process. "
                       "pread a fresh layer of suitable organic mulch such as pine bark or wood chips,"
                       " leaving a small gap around the trunk to avoid moisture buildup against the stem."
                       " Keep the mulch layer at a moderate depth so that it conserves moisture and suppresses weeds "
                       "without restricting gas exchange at the soil surface. "
                       "Refreshing the mulch helps maintain consistent soil moisture, moderates temperature extremes,"
                       " and slowly adds organic matter back into the soil as it breaks down.",
...
# For the larger model, constraining to 60 max tokens provides better results than allowing more tokens.
# This token limit achieves comparable performance across Fixed Chunking, Sentence Chunking, and Semantic Chunking strategies.